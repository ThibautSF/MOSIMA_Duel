...
explore(Time,Time2,Duration):-
	not(being_attacked(Time)),
	/*areaCovered(Radius,Size,MapWidth),*/
    (choose_exploration(Time2, Duration)
    -> jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',heightExplore,[],R),
        (jpl_is_true(R)
        -> jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',executeMountain,[],@(void))
        ; jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',executeExplore,[],@(void))
        )
    ; jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',executeLastExplore,[],@(void))
    ).
...
hunt(Life,Time,OffSize,DefSize,Radius,MapWidth,EnemyInSight):-
	...
	being_attacked(Time),
	not(EnemyInSight),
	jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',executeHunt,[],@(void)).
...
attack(EnemyInSight):-
	EnemyInSight,
	jpl_call('sma.actionsBehaviours.customs.MountainPrologBehavior',executeAttack,[],@(void)).
